...
.composer { display: grid; grid-template-columns: 1fr auto auto auto auto; gap: 10px; padding: 16px 18px 18px; border-top: 1px solid var(--border); background: #fbfdff; }
...
<div class="app">
  ...
  <section class="panel center-col">
    ...
    <div class="toolbar">
      ...
      <button class="soft" id="scoreBtn">Score call</button><button class="soft" id="speakBtn">Speak AI reply</button>
      <button class="soft" id="voiceBtn">Voice input</button><button class="soft" id="speakAgentBtn">Speak agent draft</button>
    </div>
    ...
    <div class="composer"><textarea id="agentInput" placeholder="Type the agent line here..."></textarea><button class="primary" id="sendBtn">Send</button><button id="hintBtn">Hint</button><button id="clearBtn">Clear</button><button id="objectionBtn">Prompt objection</button></div>
  </section>
...
let state={}; let lastAiMessage=''; let voiceRecognizer=null; let allVoices=[]; let selectedAiVoiceName=''; let selectedAgentVoiceName='';
...
function addMessage(type,text){const chat=document.getElementById('chat');const div=document.createElement('div');div.className=`message ${type}`;div.textContent=text;chat.appendChild(div);chat.scrollTop=chat.scrollHeight;if(type==='client')lastAiMessage=text;}
...
options.forEach(v=>{const label=`${v.name} (${v.lang})`;aiSel.add(new Option(label,v.name));agSel.add(new Option(label,v.name));});if(!selectedAiVoiceName)selectedAiVoiceName=rand(options).name;if(!selectedAgentVoiceName)selectedAgentVoiceName=options[0].name;aiSel.value=selectedAiVoiceName;agSel.value=selectedAgentVoiceName;}
...
function speakAgentDraft(){const text=document.getElementById('agentInput').value.trim();if(!text)return; speakText(text,selectedAgentVoiceName);}
...
function setupVoice(){const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){alert('Speech recognition is not supported in this browser.');return null;}const rec=new SR();rec.lang='en-US';rec.interimResults=false;rec.continuous=false;rec.onresult=(event)=>{document.getElementById('agentInput').value=event.results[0][0].transcript;};rec.onerror=()=>setStatus('Voice input had an error. You can still type manually.');rec.onstart=()=>setStatus('Listening for agent speech...');rec.onend=()=>setStatus('Voice input finished.');return rec;}
function sendAgentMessage(){const input=document.getElementById('agentInput');const text=input.value.trim();if(!text)return;if(!state.started)startCall();addMessage('agent',text);const reply=aiRespond(text);input.value='';addMessage('client',reply);setStatus(`Stage: ${state.stage}. Continue the roleplay.`);}
function forceObjection(){addMessage('client',difficultyObjection());}
document.getElementById('newScenarioBtn').addEventListener('click',startScenario);document.getElementById('startCallBtn').addEventListener('click',startCall);document.getElementById('revealMarxBtn').addEventListener('click',revealMarx);document.getElementById('openSunfireBtn').addEventListener('click',openSunfire);document.getElementById('scoreBtn').addEventListener('click',scoreCall);document.getElementById('speakBtn').addEventListener('click',speakLastMessage);document.getElementById('speakAgentBtn').addEventListener('click',speakAgentDraft);document.getElementById('sendBtn').addEventListener('click',sendAgentMessage);document.getElementById('objectionBtn').addEventListener('click',forceObjection);
document.getElementById('hintBtn').addEventListener('click',()=>addMessage('system',`Hint: ${hintText()}`));document.getElementById('clearBtn').addEventListener('click',()=>document.getElementById('agentInput').value='');
document.getElementById('voiceBtn').addEventListener('click',()=>{if(!voiceRecognizer)voiceRecognizer=setupVoice();if(voiceRecognizer)voiceRecognizer.start();});
...
document.getElementById('agentInput').addEventListener('keydown',(e)=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendAgentMessage();}});
